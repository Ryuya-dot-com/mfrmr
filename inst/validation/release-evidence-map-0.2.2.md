...
This note tracks the 0.2.2 release focus after the CRAN 0.2.1 release:
reader-first workflow refinements, observed-score G-study sensitivity,
bounded `GPCM` scope control, DIF/DFF reporting boundaries, and release
...
The 0.2.2 DIF/DFF reporting adapter continues this boundary at the direct
analysis-helper layer. `dif_report(..., style = "apa")`, `apa_table()`, and
`build_apa_outputs(..., dif_results = ...)` provide manuscript-oriented
paragraphs, tables, captions, notes, and section-map rows while preserving
the route distinction between fitted-model DFF/DIF, continuous-covariate
residual moderation, observed-score Mantel-Haenszel DIF, refit-linked
contrasts, and bounded-`GPCM` sensitivity screens. The validation helper
`inst/validation/dif-apa-reporting-0.2.2.R` checks representative cases for
method labels, table alignment, note/caption presence, bounded-`GPCM` caveats,
and absence of positive bias/fairness/invariance/subgroup-decision
overclaims. The fixed summary artifact is
`inst/validation/dif-apa-reporting-0.2.2.md`.
...
- The support statement uses "bounded `GPCM`" as a scope boundary, not as a
  synonym for a complete unrestricted GPCM implementation. The current branch
  preserves the role-based many-facet design contract, keeps
  `slope_facet == step_facet`, identifies positive slopes by a
  geometric-mean-one convention, and validates downstream helpers by capability
  row. Arbitrary slope structures, multidimensional or random-effect GPCM
  extensions, MCMC/posterior-predictive engines, and full FACETS-style
  score-side equivalence remain outside the current boundary.
- `analyze_dif_mh()` now has an observed-score Mantel-Haenszel validation
  boundary: `inst/references/mh-dif-r-package-alignment.md` records the
  distinction from fitted-MFRM routes, `mh-dif-package-comparison-0.2.2.R`
  checks the seeded `difR::difMH()` alignment fixture, and
  `mh-dif-simulation-0.2.2.R` checks direction, null behavior, matching,
  sparse-cell correction, dichotomization, and wrapper compatibility.
- `dif_report(..., style = "apa")`, `apa_table()`, and
  `build_apa_outputs(..., dif_results = ...)` now have an executable
  reporting-boundary validation artifact. `dif-apa-reporting-0.2.2.R`
  checks two-level categorical, three-level categorical,
  continuous-covariate, observed-score MH, interaction-screening, refit, and
  bounded-`GPCM` cases; `dif-apa-reporting-0.2.2.md` records the fixed
  evidence summary. `release-readiness.R` requires `DIFAPAReportingStatus =
  "ok"` and `FailedChecks = 0`.
- The fitted-MFRM DIF/DFF helpers now also have a model-crossing simulation
  smoke matrix. `dif-dff-simulation-matrix-0.2.2.R` runs seeded `RSM`, `PCM`,
  and bounded-`GPCM` cases for categorical signal, null, three-level
  categorical, and continuous-covariate DFF scenarios. The matrix checks
  target direction, screening classification boundaries, GPCM caveat carriage,
  APA wording boundaries, and draw-free plot payloads. The fixed
  `dif-dff-simulation-matrix-0.2.2.md` summary records 12 cases, 118 checks,
  and zero failed checks. `release-readiness.R` checks that
  `DIFDFFSimulationStatus = "ok"` and `FailedChecks = 0` without rerunning the
  Monte Carlo during ordinary readiness review.
- Convergence reporting now has a release-evidence status gate instead of
  only an optional stress script. `convergence-reporting-stress-0.2.2.R`
  remains outside CRAN-time tests and writes ad hoc run outputs under
  `validation-results/`. The curated
  `convergence-reporting-stress-0.2.2.md` artifact records
  `ConvergenceReportingStressStatus = "available"`, the presence/status gate,
  the direct-BFGS function-evaluation and iteration-proxy checks,
  code-0 large-gradient review rows, low-`maxit` failure-path stress, and
  MML EM iteration-basis separation. `release-readiness.R` checks the script
  and status artifact without sourcing the stress script or running fits.
- The bounded-`GPCM` score-side route now has a separate seeded smoke evidence
  artifact. `gpcm-score-side-simulation-0.2.2.R` independently verifies the
  adjacent-category kernel against package output, checks the
  `dE/dtheta = a * Var` derivative, confirms rescaling invariance and
  unit-slope PCM reduction, and writes
  `gpcm-score-side-simulation-0.2.2.md` plus summary/check CSVs. The fixed
  summary records `GPCMScoreSideSimulationStatus = "ok"`,
  `ErroredReplications = 0`, `MaxSERatioDiff = 4.441e-16`, and
  `FailedChecks = 0`. `release-readiness.R` checks those values without
  rerunning the simulation during ordinary readiness review.
- The score-side route also has a fixed external probability-kernel
  comparison artifact. `gpcm-score-side-external-comparison-0.2.2.R` maps
  `mirt::probtrace()` for `gpcmIRT` onto the local kernel with
  `tau_k = b_k`, maps TAM `tam.mml.2pl(..., irtmodel = "GPCM")` `rprobs`
  with `tau_k = beta + tau.Cat_k`, and retains `eRm::PCM()` as PCM/CML
  boundary evidence rather than a free-slope GPCM comparator. These packages
  are not treated as many-facet MFRM comparators. The fixed summary records
  `GPCMScoreSideExternalComparisonStatus = "ok"`,
  `ExternalComparisonRows = 32`, and `FailedChecks = 0`. This supports the
  package-native probability, expected-score, score-variance, and
  `dE/dtheta = a * Var` rows; it does not validate FACETS raw-score
  score-side equivalence under free discrimination.
- The 0.2.2 release scope now has a lightweight cross-document review.
  `release-scope-review-0.2.2.R` reads README, NEWS,
  CRAN comments, helper output, generated help, vignettes, source-boundary
  notes, and the fixed validation evidence files to verify that the
  comprehensive 0.2.2 scope map remains synchronized. The review now checks
  the unidimensional `fit_mfrm()` engine boundary, deferred multidimensional /
  Q-matrix / covariance / `formulaA` / mixture / response-process claims,
  G-theory as an observed-score complement rather than fitted-logit MFRM
  reliability, bounded-`GPCM`, complete-GPCM boundaries, DIF/DFF APA reporting,
  FACETS visual migration-contract columns and core visual surfaces, ETS
  display eligibility, overclaim control, and release-evidence status markers.
