...
cases <- c('null','group_mean_only','drf','interaction','combined',
  'connected_incomplete','no_common_raters','linked_null','linked_drf',
  'five_anchor_null','five_anchor_drf')
...
for(mi in 1:2) for(ci in seq_along(cases)) {
  ...
  drf <- if(case %in% c('drf','combined','connected_incomplete','no_common_raters','linked_drf','five_anchor_drf')) c(.4,-.4,0) else c(0,0,0)
  interaction <- if(case %in% c('interaction','combined')) c(.3,-.3,0) else c(0,0,0)
  n_criterion <- if(case %in% c('linked_null','linked_drf')) 3L else 2L
  if(case %in% c('five_anchor_null','five_anchor_drf')) n_criterion <- 5L
  effects <- data.frame(Group='B',Rater=paste0('R0',1:3),Effect=mean_shift+drf)
  criterion_sign <- c(1,-1,0,0,0)[seq_len(n_criterion)]
  int_effects <- expand.grid(Rater=paste0('R0',1:3),Criterion=paste0('C0',seq_len(n_criterion)),stringsAsFactors=FALSE)
  int_effects$Effect <- as.vector(outer(interaction,criterion_sign))
  thresholds <- if(model=='RSM') c(-.6,.6) else list(C01=c(-.7,.7),C02=c(-.2,.2))
  if(model=='PCM' && n_criterion>=3L) thresholds$C03 <- c(-.45,.45)
  if(model=='PCM' && n_criterion==5L) thresholds <- c(thresholds,list(C04=c(-.3,.3),C05=c(-.8,.8)))
  d <- simulate_mfrm_data(n_person=80,n_rater=3,n_criterion=n_criterion,
  ...
  ri <- match(d$Rater,paste0('R0',1:3)); ki <- match(d$Criterion,paste0('C0',seq_len(n_criterion)))
  ...
  eta <- effective_theta[d$Person]-truth$facets$Rater[d$Rater]-truth$facets$Criterion[d$Criterion] +
    (d$Group=='B')*drf[ri] + interaction[ri]*criterion_sign[ki]
  ...
  error <- tryCatch(withCallingHandlers({
    ...
    for(method in c('residual','refit')) {
      ...
      if(inherits(ans,'error')) {
        ...
        # Retain the returned table even when a following assertion fails.
        ...
        if(case=='no_common_raters') stopifnot(!any(is.finite(contrast)))
        if(case %in% c('linked_null','linked_drf') && method=='refit')
          stopifnot(nrow(tab)==3L,!any(is.finite(tab$p_adjusted)),all(tab$ScaleLinkStatus=='weak_link'))
        if(case %in% c('five_anchor_null','five_anchor_drf') && method=='refit')
          stopifnot(nrow(tab)==3L,all(is.finite(tab$p_adjusted)),all(tab$ConditionalRefitScreenEligible))
        screens[[length(screens)+1L]] <- data.frame(Cell=id,Method=method,Rows=nrow(tab),
          Available=sum(is.finite(contrast)),AdjustedPAvailable=sum(is.finite(tab$p_adjusted)),
...
stopifnot(identical(hash,tools::md5sum(files)),nrow(runs)==22L,all(runs$Error==''),
