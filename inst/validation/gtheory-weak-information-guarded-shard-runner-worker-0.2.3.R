...
if (length(file_argument) != 1L) {
  stop("The guarded runner must be invoked from one Rscript file.",
       call. = FALSE)
}
...
if (!all(file.exists(paths))) {
  stop("The guarded-runner source chain is incomplete.", call. = FALSE)
}
environment <- new.env(parent = globalenv())
for (path in paths) sys.source(path, envir = environment)
job <- readRDS(job_path)
execution <- environment$mfrmr_gtwap_worker_run(
...
if (!isTRUE(file.rename(temporary, result_path))) {
  stop("The guarded-runner result could not be atomically installed.",
       call. = FALSE)
}
