...
testlet_pilot_summarize <- function(
  ...
  check('40 unique planned datasets', nrow(status) == 40L && !anyDuplicated(status$ID) &&
  ...
  check('all attempted target rows retained', nrow(rows) == 2 * sum(plan$manifest$N + 4L) &&
  ...
  check('nonready fits keep every plugin target unavailable', all(vapply(records, function(x)
    x$status$FitReady || (!any(x$plugin$rows$Available) && nrow(x$plugin$rows) == x$spec$N + 4L), logical(1))))
  ...
  # Reuse the established ratio/cluster-MCSE helper, with all ten dataset
  ...
  for (cell in 1:4) {
    ...
    counts[[cell]] <- data.frame(Cell = cell, N = s$N[1], Variance = s$Variance[1], Planned = 10L,
    ...
    for (stratum in strata) {
      ...
      for (method in c('oracle', 'plugin')) {
        ...
        per_rep <- do.call(rbind, lapply(1:10, function(b) {
          v <- z[z$Replicate == b, ]
          valid <- v$Available
        ...
        summaries[[length(summaries) + 1L]] <- data.frame(Cell = cell, N = s$N[1], Variance = s$Variance[1],
          Stratum = stratum, Method = method, Replicates = 10L, Attempted = sum(per_rep$Attempted),
          ...
          BiasAmongAvailable = bias$estimate, RMSEAmongAvailable = sqrt(mse$estimate), PilotOnly = TRUE)
      }
      ...
      per_rep <- do.call(rbind, lapply(1:10, function(k) {
      ...
      paired[[length(paired) + 1L]] <- data.frame(Cell = cell, N = s$N[1], Variance = s$Variance[1],
        Stratum = stratum, Replicates = 10L, Matched = sum(per_rep$Matched),
        ...
        PluginMinusOracle = difference$estimate, DifferenceMCSE = difference$mcse, PilotOnly = TRUE)
    }
  }
  ...
  check('cell person/pair denominators', all(summaries$Attempted[summaries$Stratum == 'all_persons'] ==
    10 * summaries$N[summaries$Stratum == 'all_persons']) && all(summaries$Attempted[summaries$Stratum == 'pairs'] == 40L))
