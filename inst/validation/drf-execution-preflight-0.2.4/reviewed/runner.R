...
for(mi in 1:2) for(ci in seq_along(cases)) {
  ...
  drf <- if(case %in% c('drf','combined','connected_incomplete','no_common_raters','linked_drf')) c(.4,-.4,0) else c(0,0,0)
  ...
  criterion_sign <- c(1,-1,0)[seq_len(n_criterion)]
...
stopifnot(identical(hash,tools::md5sum(files)),nrow(runs)==18L,all(runs$Error==''),
