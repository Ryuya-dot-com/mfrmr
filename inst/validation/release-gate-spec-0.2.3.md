...
| Specification ID | `0.2.3-draft.44` |
| Date | 2026-08-05 |
| Status | M1 source review recorded; M2 IC, ConQuest/TAM, canonical-score, engine-path, fail-closed release-readiness, and FACETS 4.5.0 paired JML RSM/PCM stress pilots are present. The draft.20 divergence audit diagnosed exact aliases, category/step mismatch, and unmatched extreme displays. Draft.21 sequenced the corrective work; draft.22 froze the internal WP0 three-scope readiness contract. Draft.23 implements the first WP1 slice: an estimator-specific sparse constrained RSM/PCM linear-block rank audit, structured pre-optimization alias errors, counterfactual-JML review for MML population-assumption linkage, and invariance/anchor/interaction controls. Draft.24 separates MML/JML maturity; adds TAM/immer JML convention, CML/CCML reference, and HRM alternative-model lanes; and makes estimator adjustment and external package version part of comparison identity. Draft.25 instruments a bounded local observed-information review for stationary nonlinear fits, covering GPCM log slopes and active latent-regression residual variance with an explicit dense-Hessian execution limit and diagnostic-only eigenvalue tolerance ladder. Draft.26 records and numerically checks the GPCM and latent-variance free-to-expanded transformation Jacobians while marking them parameterization-only. Draft.27 combines the additive and log-slope coordinates in the exact retained JML GPCM adjacent-logit response-kernel Jacobian. Draft.28 adds a separate observed-Person-pattern marginal score matrix for nonlinear MML fits, verifies Person contributions against the full objective/gradient and central differences, and demonstrates that observed-pattern rank cannot be promoted to structural identification. Draft.29 adds bounded exhaustive response-pattern enumeration on each retained Person design, verifies probability mass and zero expected-score identities, constructs score-outer-product expected information, and checks selected pattern scores against central differences. Draft.30 canonicalizes exact Person observation designs, includes active population-design rows in their identity, evaluates each unique design once, and reconstructs expected information by exact multiplicity with a forced non-reuse equivalence control. Draft.31 begins WP2 with model-scoped declared/observed/retained/free/fixed/derived category and step records, exact sum-zero adjacent-step recession checks for zero-count internal categories, parameter-scoped statuses, and typed pre-optimization errors for unsupported free contrasts. Boundary gaps remain separate weak/element-boundary evidence; RSM local absence, PCM/GPCM ladder absence, binary no-free-step reduction, invariance, privacy, and failed-design schema controls are separated explicitly. Draft.32 begins WP3 with typed low/high primary states for standard free JML extreme Persons, separates finite optimizer traces and endpoint plot placement from the estimand, distinguishes fixed and constraint-coupled cases, preserves finite MML/EAP Persons, and removes ordinary SE/CI eligibility from typed boundaries. Draft.38 extends WP4 from fit-level propagation to the first GPCM slope parameter and uncertainty slice under contract v2. Draft.39 closes the slope-summary ambiguity and freezes a multi-layer GPCM discrepancy and stress design with estimand-specific FACETS, TAM, and immer roles. Draft.43 adds tiered replicated attribution, completeness and Monte Carlo summaries, corrects a cross-model MML EAP Person-order defect discovered under filtered rows, invalidates its pre-fix Person/diagnostic evidence, and makes optional JML audit capability part of evidence identity. Draft.44 adds atomic four-route-cell checkpoint/resume, loaded-package/runner/capability/RNG execution identity, and a hashed final-completion marker. No TAM/immer pilot has yet been run. A global or parameter-grid structural argument, broader active latent-variance and GPCM property grids, calibrated fitted-information and category-information rules, completed category/step support, generalized non-Person and interaction primary boundary states, remaining parameter/report/export/replay propagation, metric-specific external eligibility, scalable methods for exponentially large unique designs, sparse target-size benchmarks, a cross-model MML metamorphic grid, and a sufficiently replicated new-seed core remain pending. All existing simulations remain calibration evidence. Runtime completion, tolerance freeze, independent replication, recovery/coverage, bootstrap, diagnostic operating characteristics, consequence, support-envelope registry, candidate freeze, and confirmation remain pending. |
| Current draft delta | Draft.44 makes a complete four-route `DataCellId` the atomic reuse unit. Checkpoint-v1 binds selected and declared manifests, execution controls, the loaded mfrmr runtime bytes, all three internal runners, R/platform/RNG, numerical runtime reporting, and package capabilities. A temporary payload must round-trip and hash-match before rename; explicit resume rejects unexpected RDS files and every schema, execution, cell, payload, scenario, route, or declared-manifest mismatch. Aggregate outputs receive a separately validated atomic completion marker only after their hashes and the default checkpoint hashes are recorded. Deterministic interruption/resume, fresh-run equivalence, configuration mismatch, orphan partial, and artifact modification controls pass; one real reference cell agrees with the old four-route path in every result field except elapsed runtime. Absolute paths do not enter composite hashes. Draft.43 v4 remains historical and cannot be relabelled or resumed under checkpoint-v1. This closes execution resumability only: the MML metamorphic grid, core replication/precision plan, target-scale runtime, external normalization, criterion freeze, candidate freeze, and confirmation remain open. |
...
Draft.44 binds this gate to readiness contract
