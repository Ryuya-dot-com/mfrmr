...
| Specification ID | `0.2.3-draft.54` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Draft.45--49 supply MML metamorphic, portfolio, capacity, target-scale, and JML decomposition evidence; drafts.50--53 attribute and safely reduce structural and joint recession work; and draft.54 proves and implements exact shared recession geometry for the ordinary fit path. Direct/group anchors, interactions, RSM, criterion-step PCM, retained weighted/missing rows, bounded-GPCM conditional-additive geometry, malformed objects, and injected construction failure preserve legacy audit objects. The final component bundle passes all 19 fits, phase contracts, component contracts, canonical Draft.53 comparisons, and false-ready checks. Combined structural/joint time falls 36.2% and JML outer time 28.1%, with all 12 JML routes faster; this is one-replicate PCM calibration, not a frozen runtime or capacity rule. A serialized-hash defect was also corrected by type-explicit target-status canonicalization; an unchanged Draft.53 replay matches original v8 on 16 selected semantic/state fields for all 19 routes. A clean exact 492-entry local source tarball passes `R CMD check --no-manual` with `Status: OK`; this is not an `--as-cran` or candidate gate pass. Contrast construction and LP solver work are now the leading remainders. General RSM/GPCM and target-scale positive-quotient performance, replicated optimizer controls, topology/exposure controls, ADEMP recovery/precision, isolated-process capacity, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen, and confirmation remains unauthorized. |
| Current draft delta | Draft.54 adds `mfrmr-jml-recession-shared-geometry-v1`. One validated full Person-plus-structural adjacent design, target map, and contrast matrix is projected exactly for the structural audit and reused by the joint audit; invalid or failed shared construction is discarded and legacy construction is rerun. MML bypasses the path and fit schemas remain unchanged. The repository-only component profiler records nested inclusive/exclusive construction, LP, solver, and rank time only during the fit and restores namespace bindings before legacy comparison. `mfrmr-jml-target-status-canonical-v1` replaces R-internal serialized identity with type-explicit semantic identity. Draft.55 tests an exact preallocated/direct sparse observed-contrast constructor before any LP solver, warm-start, or optimizer-dispatch change. Public claims, portfolios, numeric thresholds, and confirmation authorization remain unchanged. |
