...
| Specification ID | `0.2.3-draft.56` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Drafts.45--55 establish the MML/JML computation portfolio and reduce structural/joint recession work under exact equivalence. Draft.56 now separates LP-base construction, R assembly/dispatch, and execution inside `lpSolve::lp()` without changing production dispatch. In authoritative v3, 40 bases generate 40 capacity and eight strictness solves; 8.58 of 8.64 run-LP seconds are inside the solver, versus 0.06 seconds each for all base construction and assembly/dispatch. v2 records 8.80 of 8.82 seconds inside the solver, while all classification/capacity results remain identical; this supports greater-than-99% attribution but no sub-percentage timing rule. All 19 fixed routes preserve the Draft.55 semantic/readiness/boundary baseline with zero false-ready rows and zero MML LP events. Independent sparse GLPK evaluation matches all 40 target classifications and capacities across PCM plus two RSM and two bounded-GPCM conditional-additive controls; 32 cones are negative, eight certify a direction, and maximum capacity difference is `1.421085e-14`. Thirteen sparse/dense, near-boundary, flat-direction, and forced-failure controls pass. This bounded parity is not a global nonlinear GPCM proof, solver speed comparison, dependency decision, or dispatch authorization. Alternating replicated solver qualification, status mapping, generated/scaled/tolerance properties, larger target-scale RSM/GPCM positive cones, nonlinear GPCM slope proofs, topology/exposure controls, ADEMP recovery/precision, isolated-process peak memory, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen, and confirmation remains unauthorized. |
| Current draft delta | Draft.56 adds repository-only guarded LP attribution and an independent GLPK implementation of the exact sparse/dense constraints and post-solve certificate. Independent results are computed after and never returned to production fits. `Rglpk` and `slam` remain package-external validation capabilities; `DESCRIPTION`, public APIs, fitted schemas, likelihoods, solvers, readiness, boundary decisions, public claims, and numeric thresholds do not change. The PCM-only v1 artifact is superseded by v2, which adds cross-model controls, and v2 is superseded by v3, which content-hashes the loaded mfrmr, Matrix, digest, lpSolve, slam, and Rglpk runtimes in the execution identity. Draft.57 qualifies solver candidates with alternating replicated timing, isolated-process memory, status mapping, and generated-property parity before any solver abstraction, fallback, dependency, or dispatch proposal. |
