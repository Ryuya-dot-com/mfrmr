...
| Specification ID | `0.2.3-draft.58` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Drafts.45--56 establish the MML/JML computation portfolio, reduce structural/joint recession work under exact equivalence, and locate more than 99% of observed run-LP time inside the solver. Draft.57 rejects GLPK candidacy despite 280/280 ordinary paired matches because a mathematically equivalent positive-RSM row scaling fails and neither high-level solver route preserves all requested failure classes. Draft.58 evaluates six positive/negative PCM/RSM/bounded-GPCM sources across six scale exponents, raw and L1-normalized formulations, and both solvers. All 144 rows preserve provenance; raw qualification is 66/72 because GLPK fails six larger-scale positive joint cones, while solver-only L1 row normalization qualifies 72/72 with original-scale objectives, bounds, and certificates retained. Twelve fresh child processes reproduce parent results, and four real OS-deadline controls pass. Normalization is a bounded validation candidate, but solver candidacy, dispatch, production change, runtime/capacity criteria, dependencies, and confirmation remain false or unfrozen. Larger topology/exposure-matched positive cones, nonlinear GPCM slope proofs, ADEMP recovery/precision, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen. |
| Current draft delta | Draft.58 adds a guarded solver-normalization runner and isolated worker without modifying production. It binds source, original-scale, solver-formulation, row-scale, and normalization identities; preserves the original strict objective and post-solve certificate; repeats the formerly failed RSM cell in fresh processes; and replaces mapper-only timeout evidence with actual child-process start/kill/exit provenance. `NormalizationCandidateQualified` is true only for this bounded calibration. `SolverCandidateQualified`, `ProductionChangeAuthorized`, `SolverDispatchEligible`, runtime-criterion freeze, and confirmation authorization are false. `Rglpk`, `slam`, and `processx` remain package-external validation capabilities; `DESCRIPTION`, public APIs, fitted schemas, likelihoods, solvers, readiness, boundary decisions, public claims, and numeric thresholds do not change. Solver-local expansion now stops unless model-level target-scale evidence demonstrates preventable supported-fit failures; the release spine returns to nonlinear GPCM geometry, positive RSM/GPCM cones, PCA computability, and ADEMP validation. |
