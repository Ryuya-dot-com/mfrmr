...
| Specification ID | `0.2.3-draft.55` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Draft.45--49 supply MML metamorphic, portfolio, capacity, target-scale, and JML decomposition evidence; drafts.50--54 attribute and safely reduce structural/joint recession work under exact equivalence; and draft.55 replaces repeated observed-contrast triplet growth with exact preallocation. A 977-expectation change-local test covers zero/dense/sparse designs, 1--10 steps, extremes and imbalance, malformed inputs, observation permutation, zero columns, two-Rater weighted/missing anchored PCM, interaction RSM, and bounded GPCM. The final five-case/seven-replicate constructor bundle has exact output identity; median time falls 85.7--99.7% and cumulative R allocation 93.3--99.96%, but neither is a frozen performance or capacity rule. The fixed component bundle passes all 19 fits, phase/component contracts, canonical comparisons, and false-ready checks. Against Draft.54, contrast time falls 98.9%, structural-plus-joint time 49.5%, and JML outer time 32.7%; all 12 JML routes are faster and MML is unaffected. A clean exact 493-entry local source tarball passes `R CMD check --no-manual` with `Status: OK`; its CRAN-light path has 397 passing expectations and three skips. A separate hash-verified 10-shard regression over the same tarball's complete 126-file test inventory records 1,726 tests, 11,945 passing expectations, 83 skips, 38 allowlisted warnings, and zero failures, errors, or unexpected warnings. A monolithic full-regression attempt timed out at 30 minutes and is not called a result; the sharded pass is not a second `R CMD check`. This remains below `--as-cran` and candidate gates. LP solver calls now dominate measured recession work. General target-scale RSM/GPCM and positive-quotient performance, independent solver parity, replicated fit controls, topology/exposure controls, ADEMP recovery/precision, isolated-process peak memory, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen, and confirmation remains unauthorized. |
| Current draft delta | Draft.55 retains the former observed-contrast algorithm as an internal reference and makes score-specific transition templates plus exact triplet preallocation the ordinary path. Both paths build the same sparse operator and matrix product; public APIs, fitted schemas, likelihoods, solvers, readiness, and boundary decisions do not change. Repository-only timing and `Rprofmem()` are diagnostic-only. The promoted v3 constructor evidence removes staging-path row-name leakage found in v1 and uses the established Draft.49--54 installed-runtime identity convention. Draft.56 attributes LP formulation and repeated-solve work before considering reusable models, warm starts, alternative solvers, or dispatch. Public claims, portfolios, numeric thresholds, and confirmation authorization remain unchanged. |
