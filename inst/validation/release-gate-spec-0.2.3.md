...
| Specification ID | `0.2.3-draft.35` |
| Date | 2026-08-04 |
...
| Current draft delta | Draft.35 adds a globally screened joint Person-structural additive recession cone while retaining the draft.33 Person-fixed structural certificate. Individual Person targets are limited to constraint-coupled extremes unresolved by the sufficient-score rule, but all free Person coordinates enter the cone. Target-direction limits apply only after a global ray is certified, so a no-ray result does not enumerate per-target LPs. A group-constrained two-Person/two-Item adversarial fixture has no Person-only or structural-only ray and a certified joint ray; sparse/dense LPs, row reversal, a nonseparated control, coordinate and post-screen target limits, MML non-reduction, and a test-only exhaustive finite-grid oracle agree. This is bounded linear-cone evidence only: general independent solver parity, end-to-end target-size benchmarks, public primary-value propagation, nonlinear GPCM slope recession, and all confirmation evidence remain pending. |
