...
| Specification ID | `0.2.3-draft.33` |
| Date | 2026-08-04 |
...
| Current draft delta | Draft.33 adds a bounded Person-fixed additive structural recession certificate using the retained observed-versus-alternative category contrasts and exact optimizer constraint map. It covers non-Person facet levels, two-way interaction cells, and step targets in both directions when the optional LP solver is available. Candidate directions are post-solve certified and stored internally only; public primary-value propagation, joint Person movement, nonlinear GPCM slope recession, sparse target-size execution, independent solver parity, and all confirmation evidence remain pending. |
