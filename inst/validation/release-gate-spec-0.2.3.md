...
| Specification ID | `0.2.3-draft.50` |
| Date | 2026-08-05 |
| Status | M1 is recorded and M2 remains in progress. Draft.45 supplies a 30/30 small-design MML metamorphic pilot; draft.46 separates the 87-row inventory into release-spine, claim-conditional, and deferred work; draft.47--49 execute mixed-adversity, baseline/bridge, and JML bottleneck slices. Draft.50 adds decision-nonintervening phase timing and reprofiles 19 fixed Draft.49 routes. All routes executed, all timing contracts held, zero false-ready rows occurred, and all semantic-result hashes/readiness/optimizer states matched the progressively refined v1--v3 runs. Structural and joint recession LP audits consumed about 95.6% of instrumented JML time; optimizer work consumed about 1.8%. The 200-parameter dispatch remains a numerical-readiness hypothesis but is not the primary performance explanation. A structural global-cone prescreen now precedes wider optimizer/performance work, subject to exact certificate and fail-closed equivalence. No evidence row or numerical threshold is passed/frozen, and confirmation remains unauthorized. Replicated cross-model optimizer controls, topology/exposure-matched controls, replicated ADEMP recovery/precision, isolated-process capacity, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. All simulations remain calibration evidence. |
| Current draft delta | Draft.50 adds an internal opt-in 18-phase timer whose rows are attached after readiness and are labelled diagnostic-only; ordinary fits retain no timing field. A fixed seven-cell/19-route PCM profile covers P200/P400, R12, C12-E04/E02/E12, forced extremes, paired MML routes, and selected BFGS controls. Across 12 JML routes, structural recession auditing took 139.63 seconds, joint recession auditing 62.31 seconds, and optimization 3.70 seconds out of 211.25 instrumented seconds. Every structural audit returned `none_certified` after enumerating 46--126 target directions; the joint audit already avoids target enumeration when its cone screen is negative. The next change-local hypothesis generalizes an exact global-cone prescreen to the structural audit, preserving every solver, size, certificate, boundary, and readiness contract. Coarse v1 and refined v2 remain transparent diagnostics; authoritative v3 adds workload dimensions and matches all 19 semantic results. Public claims, release portfolios, numerical thresholds, and confirmation authorization remain unchanged. |
...
optimizer accounts for only a small JML share. The next change-local slice
tests a mathematically equivalent structural global-cone prescreen before
target enumeration; it may not bypass certificate or fail-closed states. The
full records are `jml-bottleneck-decomposition-pilot-record-0.2.3.md` and
