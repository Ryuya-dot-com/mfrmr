...
| Specification ID | `0.2.3-draft.51` |
| Date | 2026-08-05 |
| Status | M1 is recorded and M2 remains in progress. Draft.45 supplies a 30/30 small-design MML metamorphic pilot; draft.46 separates the 87-row inventory into release-spine, claim-conditional, and deferred work; draft.47--49 execute mixed-adversity, baseline/bridge, and JML bottleneck slices; draft.50 attributes selected JML runtime. Draft.51 implements the guarded structural global-cone prescreen and reruns the fixed 19-route profile. All routes execute, all timing contracts hold, zero false-ready rows occur, all v3/v5 semantic/readiness/boundary states match, and all 12 same-fit screened/unscreened structural target-status hashes match. The prescreen replaces 908 legacy target LP calls with 12 cone LP calls and zero target LP calls in these negative-cone PCM routes; structural phase time falls 90.1% and JML outer time 59.3%. Adversarial review adds a near-boundary counterexample and separates the `1e-10` cone exclusion tolerance from the `1e-7` target certificate tolerance; pre-guard v4 is superseded by v5. This is change-local, one-replicate calibration evidence, not a frozen capacity rule. Joint recession attribution, positive-cone target-scale evidence, RSM/GPCM coverage, replicated optimizer controls, topology/exposure controls, ADEMP recovery/precision, isolated-process capacity, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen, and confirmation remains unauthorized. |
| Current draft delta | Draft.51 adds `mfrmr-jml-global-cone-prescreen-v1` to the internal Person-fixed structural recession audit. A negative screen proves that no strictly improving observed-category contrast row exists and returns the same finite target classifications without target enumeration; a positive screen retains the legacy positive/negative target LPs. The result records prescreen state, the guarded objective/certificate tolerances, and actual LP calls, while dependency, design, mapping, solver, size, anchor, interaction, retained-row, sparse/dense, row-order, and MML guards remain fail closed. The authoritative v5 bundle compares every JML fit with an unscreened re-audit of the same object and compares all 19 routes with v3 and v4. The next slice attributes joint cone versus target work before considering shared construction, reuse, warm starts, or solver changes. Public claims, release portfolios, numerical thresholds, optimizer dispatch, and confirmation authorization remain unchanged. |
