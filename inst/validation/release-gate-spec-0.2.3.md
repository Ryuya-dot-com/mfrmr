...
| Specification ID | `0.2.3-draft.40` |
| Date | 2026-08-05 |
...
| Current draft delta | Draft.40 adds a bounded joint JML GPCM path family in which constrained additive coordinates move linearly while an ordered pair of expanded log slopes has rates `+1` and `-1`. Sparse linear-program conditions, independently reconstructed retained and analytic boundary likelihoods, direct path checks, positive and negative adversarial fixtures, row-order invariance, workload limits, and MML non-reuse are enforced. A competitive path receives candidate-specific readiness reasons but remains `not_evaluated`, without a primary value or ordinary uncertainty, because this sufficient family does not resolve the globally non-concave GPCM likelihood. More general joint paths, marginal-MML geometry, full parameter/output propagation, executable covering-grid runners, frozen numeric criteria, candidate-linked external runs, and all confirmation evidence remain pending. |
...
Draft.40 binds this gate to readiness contract
