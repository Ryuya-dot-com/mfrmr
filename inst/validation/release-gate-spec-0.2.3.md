...
| Specification ID | `0.2.3-draft.60` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Drafts.45--56 establish the MML/JML computation portfolio and exact guarded reductions; Drafts.57--59 reject GLPK and target-scale normalization while exposing a native-timeout replay defect. Draft.60 binds seven exact Draft.59 problems to four policies and four order-balanced fresh-process repetitions. All 112 workers complete without a parent kill and all final results are safe, but the current two-second policy has only six of seven stable cells and five of seven matches to the OS-bounded native-zero reference. Problem 8 changes between one capacity failure and three strictness failures; problem 13 repeats a strictness failure four times despite four reference certificates. The bounded two-then-eight-second retry and bounded single-ten-second policies each produce seven of seven stable, reference-matched, negative-control-preserving cells. Both advance only to fit-level continuation, so policy selection, production change, runtime/capacity freeze, replay-blocker closure, and confirmation remain false. Fit-level paired policy evidence, nonlinear GPCM slope proofs, ADEMP recovery/precision, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen. |
| Current draft delta | Draft.60 adds a guarded replay runner and isolated worker without modifying production. It journals capacity and strictness separately, gives every execution a unique problem/policy/replicate key, rotates execution order, retains identity-bound resumable checkpoints, and places an independent parent deadline around every worker. Status 0 is necessary but insufficient: split-box, original-primal, objective-reconstruction, capacity-bound, target-floor, and original-scale certificate checks remain mandatory. Across 198 stage attempts, two capacity and thirteen strictness calls at two seconds return status 1 and fail closed; all seven required eight-second retries succeed. Both bounded policies are pilot-qualified, but the tie cannot be broken by the selected-problem elapsed times. `BoundedPolicyPilotQualified` and `PolicyContinuationAuthorized` are true; `PolicySelectionAuthorized`, `ProductionChangeAuthorized`, `ReplayBlockerResolved`, runtime freeze, and confirmation authorization are false. `processx` remains validation-only; `DESCRIPTION`, public APIs, fitted schemas, likelihoods, solvers, readiness logic, boundary rules, public claims, and numeric thresholds do not change. Draft.61 must compare both policies at full-fit level under finite audit deadlines and a prespecified consequence/workload rule. |
