...
| Specification ID | `0.2.3-draft.62` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Drafts.45--56 establish the MML/JML computation portfolio and exact guarded reductions; Drafts.57--59 reject GLPK and target-scale normalization while exposing a native-timeout replay defect; Drafts.60--61 qualify and select the bounded single-ten-second policy. Draft.62 implements that policy only for additive structural and joint JML recession audits and binds its retention to native six-route identity, affected regression, and exact-tarball package evidence. The narrow replay blocker is resolved, but no runtime/capacity envelope or checklist criterion is frozen. Nonlinear GPCM slope proofs, ADEMP recovery/precision, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen. |
| Current draft delta | Draft.62 adds the versioned internal contract `mfrmr-jml-recession-fit-policy-v1`: one native ten-second attempt per additive capacity or strictness stage, no retry after an unaccepted solve, and at most 20 native seconds for a positive target requiring both stages. The nonlinear GPCM joint-pair audit retains its separate two-second default. Eighteen fresh production-native fits over the six Draft.59 RSM/GPCM routes complete safely with zero parent kills, 51 ten-second target calls, 66 solver attempts, six of six stable route cells, and exact selected-candidate result and call-outcome identity in all 18 matched cells. The final tarball SHA-256 is `7ceb2848958f2f70084e987342eb3e58e6a38b2b41964ceb0d385ea276d92624`; its sharded full non-CRAN regression covers 129/129 files with 13,172 passing expectations, 38 expected review warnings, 23 optional skips, and zero failures/errors. Its temporary-filesystem `R CMD check --no-manual` completes code, tests, examples, and vignettes with one offline optional-Suggests Rd-cross-reference NOTE and no WARNING; it is not a complete-dependency, manual, `--as-cran`, or `Status: OK` check. `ReplayBlockerResolved=true` applies only to the additive JML recession replay defect. Runtime/capacity freeze, checklist promotion, and confirmation authorization remain false; elapsed time is not a selection or acceptance criterion. `DESCRIPTION`, exported APIs, fitted schemas, likelihoods, optimizers, readiness logic, boundary semantics, public claims, and numeric statistical thresholds do not change. |
