...
| Specification ID | `0.2.3-draft.41` |
| Date | 2026-08-05 |
...
| Current draft delta | Draft.41 implements the repository-only GPCM stress covering grid. A deterministic mixed-level generator retains 12 mandatory corners and 70 pilot cells that cover all 1,330 two-axis level combinations across estimator/lower-model route, slope count and spread, categories and prevalence, rater panel, assignment, missingness, cell structure, interactions, diagnostics, and sample size. It separates `PCM_JML` and `PCM_MML` references, partitions smoke/pilot/confirmation seeds, records the unsupported genuine one-slope-level generator as a known gap, hashes every retained dataset, and reports support, readiness, boundary, primary-versus-optimizer slope, false-ready, and optional residual-PCA fields. External numeric comparisons remain ineligible, numeric criteria remain unfrozen, and every current result is calibration-only. Full pilot replication, recovery/coverage and Monte Carlo criteria, target-scale runtime/memory, matched external normalizers, general joint paths, marginal-MML geometry, candidate freeze, and confirmation remain pending. |
...
Draft.41 binds this gate to readiness contract
