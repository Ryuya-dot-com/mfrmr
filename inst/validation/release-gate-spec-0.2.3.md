...
| Specification ID | `0.2.3-draft.61` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Drafts.45--56 establish the MML/JML computation portfolio and exact guarded reductions; Drafts.57--59 reject GLPK and target-scale normalization while exposing a native-timeout replay defect; Draft.60 qualifies two bounded problem-level policies. Draft.61 compares the current policy, both candidates, and an OS-bounded native-zero reference over the six complete/sparse RSM/GPCM Draft.59 fits and three fresh-process repetitions. All 72 workers and fits complete safely with zero parent kills and invariant optimizer hashes. The current policy is stable on five of six routes and reference-matched on only four. Both bounded candidates are stable and reference-matched on all six. Under the prespecified equal-20-second maximum positive-target bound, single-ten-second uses no more solver attempts in all 18 paired cells and fewer in six, so it alone advances as an implementation candidate. Production change, runtime/capacity freeze, replay-blocker closure, checklist promotion, and confirmation remain false. Candidate implementation/revalidation, nonlinear GPCM slope proofs, ADEMP recovery/precision, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen. |
| Current draft delta | Draft.61 adds a guarded full-fit runner and isolated worker without modifying production. It reconstructs six exact Draft.59 inputs, captures every target LP call after optimization, rotates four policy orders over three fresh-process repetitions, retains exact input and optimizer identities, and places independent finite parent deadlines around every fit. All 72 fits are safe; 23/24 policy-by-route cells are stable. Balanced-sparse RSM exposes a stable current-policy false negative and random-sparse RSM exposes instability, while both candidates reproduce all six reference outcomes. Their target-call sequences agree in 18/18 paired cells; at the same declared maximum native bound, single-ten-second requires 66 attempts versus retry's 77, is noninferior in 18/18 cells, and strictly lower in six. The frozen consequence rule therefore sets `SelectedImplementationPolicy=bounded_single_10s` and `ImplementationCandidateSelected=true`. `ProductionChangeAuthorized`, `ReplayBlockerResolved`, runtime freeze, checklist promotion, and confirmation authorization remain false. Elapsed time cannot select or freeze a rule. `processx` remains validation-only; `DESCRIPTION`, public APIs, fitted schemas, likelihoods, solvers, readiness logic, boundary rules, public claims, and numeric thresholds do not change. Draft.62 must implement the candidate minimally and reproduce affected native unit, target-scale, regression, and package-check evidence. |
