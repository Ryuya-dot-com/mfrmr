...
| Specification ID | `0.2.3-draft.57` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Drafts.45--56 establish the MML/JML computation portfolio, reduce structural/joint recession work under exact equivalence, and locate more than 99% of observed run-LP time inside the solver. Draft.57 reacquires the exact 40-target PCM/RSM/bounded-GPCM conditional-additive ledger and runs one excluded warm-up plus seven included balanced alternating-order replicates per solver. All 560 timed calls match their production result and all 280 pairs agree. GLPK totals 13.32 seconds versus 60.05 for `lpSolve`, but the candidate is rejected: only 94 of 96 generated property rows pass because GLPK returns status 1 for a positive RSM cone after mathematically equivalent `1e-3`--`1e3` row scaling while `lpSolve` retains certified capacity 147. Status controls safely reject every unsafe result but show that `lpSolve` can return status 0 and `1e30` for an unbounded fixture and Rglpk does not distinguish infeasible from unbounded failure. Six isolated PCM/RSM/GPCM processes complete, but process-lifetime peaks freeze no memory rule. Candidate, dispatch, runtime, memory, dependency, and confirmation states remain unpromoted. Solver-neutral normalization/status provenance, actual isolated timeout handling, larger target-scale positive cones, nonlinear GPCM slope proofs, topology/exposure controls, ADEMP recovery/precision, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen. |
| Current draft delta | Draft.57 adds a guarded solver-qualification runner and isolated worker without modifying production dispatch. Candidate qualification is separated from evidence completion so the authoritative negative result is retained. Forty captured targets reproduce Draft.56 exactly; the 280 ordinary paired comparisons all pass, but the frozen scale property and failure-status-specificity requirements do not. `CandidateQualified`, `SolverDispatchEligible`, runtime/memory criteria, and confirmation authorization are false. `Rglpk`, `slam`, and `ps` remain package-external validation capabilities; `DESCRIPTION`, public APIs, fitted schemas, likelihoods, solvers, readiness, boundary decisions, public claims, and numeric thresholds do not change. Draft.58 investigates identity-bound solver-row normalization and OS-deadlined status provenance before any further solver proposal. |
