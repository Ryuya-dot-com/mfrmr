...
| Specification ID | `0.2.3-draft.59` |
| Date | 2026-08-06 |
| Status | M1 is recorded and M2 remains in progress. Drafts.45--56 establish the MML/JML computation portfolio, reduce structural/joint recession work under exact equivalence, and locate more than 99% of observed run-LP time inside the solver. Draft.57 rejects GLPK candidacy, and Draft.58 qualifies solver-only L1 row normalization only on a bounded six-source calibration. Draft.59 extends the audit to three topology- and exposure-matched 400-Person RSM/GPCM designs. All six ordinary and capture fits return objects and fail closed, but only five preserve identical semantic, readiness, and boundary content: one random-sparse RSM joint audit changes from `not_evaluated_solver` to `certified_recession` on identical data and arguments. All 64 raw/normalized and `lpSolve`/GLPK rows are safe, only 61 preserve internal raw-to-target provenance, and raw versus normalized `lpSolve` qualification is 15/16 versus 13/16. Ten selected timeout-zero attribution rows are safe and certified, while three change the two-second outcome. Target normalization and a production need for it are rejected; native-timeout instability and a recession-replay investigation are recorded. Solver candidacy, dispatch, production change, runtime/capacity criteria, dependencies, and confirmation remain false or unfrozen. Fresh-process bounded-policy replay, nonlinear GPCM slope proofs, ADEMP recovery/precision, PCA computability, estimator-specific weak-information rules, metric-specific external eligibility, candidate identity, confirmation, and release engineering remain pending. No checklist row or numeric criterion is passed/frozen. |
| Current draft delta | Draft.59 adds a guarded target-positive-cone runner without modifying production. It binds three 400-Person assignment/exposure structures shared exactly between RSM and bounded GPCM, requires distinct response hashes, pairs ordinary and post-call capture fits, serializes the 16 exact recession problems, and compares raw and L1-normalized formulations through `lpSolve` and GLPK on the original certificate scale. The evidence-completion rule is deliberately separated from candidate qualification, so the five-of-six fit-replay match and three provenance failures are retained rather than discarded. A prespecified post-failure timeout-zero reference localizes three changed outcomes but cannot authorize unlimited production execution. `ProductionNormalizationNeedObserved` and `TargetNormalizationQualified` are false; `NativeTimeoutInstabilityObserved` and `RecessionReplayInvestigationRequired` are true. `ProductionChangeAuthorized`, `SolverBranchContinuationAuthorized`, `SolverDispatchEligible`, runtime-criterion freeze, and confirmation authorization are false. `Rglpk` and `slam` remain package-external validation capabilities; `DESCRIPTION`, public APIs, fitted schemas, likelihoods, solvers, readiness logic, boundary rules, public claims, and numeric thresholds do not change. Draft.60 must isolate capacity and strictness stages in fresh processes under bounded retry/reference policies before target-scale or statistical validation resumes. |
