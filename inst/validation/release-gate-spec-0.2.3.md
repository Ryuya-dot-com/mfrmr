...
| Specification ID | `0.2.3-draft.34` |
| Date | 2026-08-04 |
...
| Current draft delta | Draft.34 replaces the structural certificate's dense constraint allocation with sparse row-column-value triplets, retains a bounded dense-reference formulation, records sparse and dense-equivalent workloads, adds constraint/coordinate/nonzero/direction execution ceilings, and checks sparse/dense parity plus an independent finite-grid oracle on low-dimensional Rater and interaction microcases. This is allocation and formulation evidence only: general independent solver parity, end-to-end target-size benchmarks, public primary-value propagation, joint Person movement, nonlinear GPCM slope recession, and all confirmation evidence remain pending. |
