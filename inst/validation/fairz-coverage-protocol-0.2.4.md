...
## Source-contract hardening after the first preflight

The first 40 preflight datasets and eight full q121 checks passed. Before
confirmation, inspection identified that the default direct MML path also
uses the compiled cpp11 backend. The original R-only snapshot was insufficient
to identify that execution source. Add C/C++/header/Makevars sources to the
payload; record and check the loaded native-library MD5, cpp11-enabled option,
R version and platform at resume/checkpoint. A different binary/platform needs
its own matching preflight; no untested cross-platform equivalence is assumed.

Preserve the initial runner/protocol/checks and outputs in
`fairz-coverage-0.2.4/initial-source-contract/`. Repeat the same 40 preflight
seeds under the strengthened source contract, without changing generation,
estimation, SEs, intervals, counts or statistical criteria. These remain
40 unique preflight datasets, not 80 independent replications; confirmation
seeds have not been used. Check the repeated datasets/results directly.
