...
| `WP6-SCALE-AND-ADVERSARIAL` | affected WP1--WP4 slice | `in_progress_target_replay_blocker_normalization_rejected` | Verify sparse computation, basis invariance, row-order and label invariance, malformed-input behavior, optional-capability fail-closed behavior, and target-size runtime/memory without claiming FACETS capacity parity. Drafts.43--56 establish execution identity, metamorphic/capacity baselines, guarded exact reductions, sparse construction, and solver attribution. Draft.57 rejects GLPK candidacy; Draft.58 qualifies only bounded six-source L1 normalization. Draft.59 pairs three 400-Person RSM/GPCM topology and exposure designs: 64/64 comparison rows are safe, but provenance is 61/64, raw/normalized `lpSolve` qualification is 15/16 and 13/16, target normalization is rejected, one random-sparse RSM ordinary/capture pair changes joint state, and timeout-zero changes 3/10 diagnostic outcomes. Keep solver selection closed and treat two-second native-timeout replay as a distinct release-spine blocker. Next isolate capacity/strictness stages over frozen positive, failed, and negative RSM problems with a fresh-process bounded-policy ladder; then return to nonlinear GPCM, PCA computability, serialization/replay, and broader active-structure grids. | Benchmark envelope and metamorphic/negative-test report; capability manifest; no dense design allocation at target sizes. |
| `WP7-REPILOT-AND-FREEZE` | frozen release-spine profile plus affected WP0--WP6 slices | `in_progress_replay_policy_calibration_required` | Prespecify replication counts, MCSE targets, failure denominators, seeds, and manifests in parallel. Calibration pilots may run only for explicitly stable slices and are invalidated by later affected code/contract changes. Drafts.43--58 supply feasibility, resumability, invariance, computation attribution, guarded reductions, negative solver qualification, bounded normalization, and OS-deadline controls. Draft.59 adds topology/exposure-matched target-scale evidence but rejects normalization and exposes execution-sensitive RSM recession classification: five of six ordinary/capture routes match, all 64 solver results are safe, only 61 preserve internal provenance, and timeout-zero changes three of ten selected outcomes. No candidate or runtime/capacity rule can freeze while replay depends on a two-second native limit. Draft.60 must prespecify fresh-process repetitions and bounded retry/reference policies without accepting status alone; general nonlinear GPCM, replicated fit timing, PCA criteria, ADEMP recovery/coverage, external eligibility, and the complete statistical precision plan remain open. Confirmation stays unauthorized until replay, profile, criteria, and candidate identity are frozen. | Claim-disposition profile, complete pilot registry, atomic resumable execution, method-mode-specific exclusions, prespecified precision plan, resolved release-spine blockers, and still no confirmation result. |
