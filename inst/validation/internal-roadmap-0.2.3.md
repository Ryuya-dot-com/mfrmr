...
| `WP6-SCALE-AND-ADVERSARIAL` | affected WP1--WP4 slice | `in_progress_two_bounded_replay_policies_fit_level_selection_required` | Verify sparse computation, basis invariance, row-order and label invariance, malformed-input behavior, optional-capability fail-closed behavior, and target-size runtime/memory without claiming FACETS capacity parity. Drafts.43--56 establish execution identity, exact guarded reductions, sparse construction, and solver attribution; Drafts.57--59 reject GLPK and target normalization while exposing native-timeout replay failure. Draft.60 runs seven frozen problems under four policies and four fresh-process repetitions: 112/112 workers and safe results, zero parent kills, 27/28 stable cells, and only 5/7 production-reference matches. The bounded retry and single-ten-second policies each qualify 7/7 for continuation, but neither is selected. Draft.61 must compare both at full-fit level under independent deadlines, bind all-target workload and boundary/readiness propagation, and freeze neither production nor runtime from problem-level elapsed time. Then return to nonlinear GPCM, PCA computability, serialization/replay, and broader active-structure grids. | Benchmark envelope and metamorphic/negative-test report; capability manifest; no dense design allocation at target sizes. |
| `WP7-REPILOT-AND-FREEZE` | frozen release-spine profile plus affected WP0--WP6 slices | `in_progress_fit_level_replay_policy_replication_required` | Prespecify replication counts, MCSE targets, failure denominators, seeds, and manifests in parallel. Calibration pilots may run only for explicitly stable slices and are invalidated by later affected code/contract changes. Drafts.43--59 supply resumability, invariance, computation attribution, exact reductions, solver-negative evidence, bounded normalization, and the target-scale replay blocker. Draft.60 separates capacity/strictness and completes 112 fresh processes with 198 stage attempts: current two-second replay is unstable or reference-inconsistent on problems 8 and 13; both bounded candidates are stable, reference-matched, and negative-control preserving. This authorizes only a two-policy fit-level continuation. Draft.61 must prespecify fit repetitions, semantic/optimizer invariance, full boundary/readiness outcomes, audit-level deadlines, target-call denominators, and a tie-breaking consequence rule before one candidate can be frozen. General nonlinear GPCM, replicated fit timing, PCA criteria, ADEMP recovery/coverage, external eligibility, and the complete statistical precision plan remain open. Confirmation stays unauthorized until replay, profile, criteria, and candidate identity are frozen. | Claim-disposition profile, complete pilot registry, atomic resumable execution, method-mode-specific exclusions, prespecified precision plan, resolved release-spine blockers, and still no confirmation result. |
