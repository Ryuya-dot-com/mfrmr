...
This bundled note is the validation excerpt for bounded-`GPCM` work that
remains caveated, blocked, or deferred after the 0.2.2 release boundary. It
also records complete-`GPCM` exit criteria that are read by release-scope
checks. It is intentionally narrower than the full project roadmap.

This is a validation evidence artifact rather than an analysis guide. The
current user-facing contract remains `gpcm_capability_matrix()`,
`mfrmr_model_family_scope()`, and `mfrmr_estimation_scope()`.

Package sequencing note: restricted multidimensional `RSM`/`PCM` is planned
before bounded-`GPCM` validation expansion. That multidimensional work is
separate from the complete-`GPCM` conditions below and must not be used to
imply multidimensional `GPCM` support. The synchronized package sequence is:
0.2.3 for restricted multidimensional `RSM`/`PCM`, 0.2.4 for bounded-`GPCM`
validation expansion, 0.2.5 for a general unidimensional `GPCM` route, and
0.2.6 or later for posterior predictive checks and heavy-backend bridges.
...
items here; the remaining entries describe post-0.2.2 work or package release
preparation.
...
These are package release tasks, not model-family work.

- Re-run full source-package checks after the final documentation and
  validation-source edits, rather than relying on earlier 0.2.2 stabilization
  logs.
- Refresh `release_readiness()` after the source-package check so the bundled
  evidence, current `DESCRIPTION` version, and local check log agree.
- Keep release notes, citation metadata, and any archival release tag aligned
  with the exact package version being released.

### Model-family and diagnostic priorities

- Within the `GPCM` lane, complete unrestricted `GPCM` remains the highest
  non-current target. It is the most concrete externally verifiable gap against
  packages that fit free discrimination GPCM routes. This does not override
  the consolidated package sequence that places restricted multidimensional
  `RSM`/`PCM` before bounded-`GPCM` validation expansion.
...
- Add residual-diagnostic stress cells that separate local dependence from
  slope/threshold misspecification, sparse category support, and weak links;
  Q3-style or pairwise residual flags remain exploratory until calibrated by
  replicated-data or bootstrap evidence.
- Extend DFF/DRF evidence with source-grounded simulation cells before making
  stronger subgroup claims: severity-only effects, centrality/range-restriction
  effects, dual DRF, null cases, group imbalance, sparse/common-link designs,
  and slope-regime variation should be separated rather than collapsed into one
  generic DFF scenario.
...
- Include replicated Q3-style local-dependence and residual-PCA discrepancy
  measures so fixed residual-correlation thresholds are not treated as
  calibrated tests.
