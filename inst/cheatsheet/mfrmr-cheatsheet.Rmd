...
summary(apa)                 # QA / completeness check
cat(apa$report_text)
...
## 5. Hierarchical structure and small-N audit
...
| `plot_person_fit(fit, diagnostics = diag)` | Per-person Infit/Outfit bubble; inspect `p$data$data$MisfitDirection` for underfit vs overfit |
| `fit_p_table(fit, diagnostics = diag)` | TAM-style Infit/Outfit p-value table with adjusted p values |
| `plot_empirical_fit(fit, diagnostics = diag)` | mirt-style observed-vs-expected empirical fit bins |
...
| `plot_fair_average(fit, show_ci=TRUE)` | Fair averages + delta-method CI |
...
# $hierarchical_audit, $missing_recoding, $shrinkage_audit
replay_script <- build_mfrm_replay_script(fit)
writeLines(replay_script$script, "replay_mfrmr.R")
```
...
- Documentation: <https://github.com/Ryuya-dot-com/mfrmr>
