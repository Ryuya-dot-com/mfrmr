...
level-specific discriminations and category steps. GPCM MML
...
This README describes the current `0.2.4` source, including portable
calibration, exploratory external-feature groups, numeric PCA/k-means,
assigned-score imputation, fixed-facet sandwich intervals, screening
evaluation, multivariate G/D-studies, and shared random-rater and
Person-specific testlet RSMs. See [the
roadmap](https://ryuya-dot-com.github.io/mfrmr/ROADMAP.html) for
supported scope and future work. The candidate is intended for
evaluation and has not been released on CRAN. Functions and options
shown here may differ from an installed release; retain the installed
source tag or commit and use its matching help. Earlier candidates also
report `0.2.4`, so `packageVersion("mfrmr")` alone cannot distinguish
them. For an existing analysis, read [Updating saved
analyses](#updating-saved-analyses) before reusing saved diagnostics,
scores or reports.
...
To use the examples below with this source archive, install the
downloaded file (replace the path with its location on your computer):
...
and `category_policy`; their older equivalents are
...
and `keep_original`. The `"feedback"` output-guide scope also requires
the current source. Use the current source archive or checkout for these
additions. To install that earlier published candidate reproducibly:
...
provides the rc.5 source archive with prebuilt tutorials, its checksum
...
workflows are described separately. Unsupported combinations and
inference states are reported explicitly.
...
The GPCM estimates **relative discriminations and category steps for the
same selected facet**:
