...
specialized tables, reviews, simulations, or compatibility outputs.
...
The reporting helpers are designed to get a user to a defensible draft,
not to replace statistical judgment or journal-specific writing. A
manuscript-ready route normally means:

- use `fit_mfrm(method = "MML")` and record the model, facets, score
  scale, quadrature, anchors, and missing-data decisions;
- build `res <- mfrm_results(fit, include = "publication")`, then
  inspect `summary(res)$triage`, `summary(res)$status`, and
  `summary(res)$next_actions`;
- build `report <- mfrm_report(res, style = "apa")` and review
  `report$first_screen`, `report$claim_readiness`, `report$report_gaps`,
  and `report$template_index` before copying any wording;
- use
  ...
  and `export_mfrm_results(res, include = c("default", "report"))` for
  tables, captions, notes, and reproducible handoff files;
- treat `DraftReady`, `ReadyForAPA`, and report templates as
  drafting-readiness indicators, not as automatic proof that a claim is
  strong enough for a particular journal.

For a high-stakes journal article, the package can supply the
measurement model output, diagnostic evidence, visual evidence,
APA-style scaffolding, and reproducible export trail. The final paper
still needs the study design, measurement rationale, source citations,
substantive interpretation, sensitivity decisions, and journal-specific
narrative written by the analyst.
...
`preset = "monochrome"` for grayscale / line-type-friendly output. When
a journal, accessibility review, or house style requires a different
visual grammar, use `draw = FALSE`,
[`plot_data_components()`](https://ryuya-dot-com.github.io/mfrmr/reference/plot_data_components.md),
and
[`plot_data()`](https://ryuya-dot-com.github.io/mfrmr/reference/plot_data.md)
to build your own base-R, ggplot2, plotly, Quarto, or Shiny display.
Keep the returned `guidance`, `reference_lines`, `category_support`,
`interpretation_guide`, and reporting-template rows with the custom
figure so the caption does not lose the original interpretation
boundary.

## Reproducible examples and validation evidence

The CRAN release is the baseline for examples and external review.
Lightweight examples use the bundled `example_core` data so the public
route stays quick to run, while heavier validation, simulation, and
CRAN-check evidence can be generated separately. The core reproducible
route is the same public route recommended above:
...
Longer-running checks are intentionally kept out of first-contact
examples. They are regenerated by explicit scripts and summarized
through saved `.rds` and `.csv` artifacts. The workflow vignette uses
small generated CSV artifacts to show representative output during
CRAN-style builds while keeping the live calculation route available
with `NOT_CRAN=true`.
...
|----|----|----|
| New reproducible analysis | [`fit_mfrm()`](https://ryuya-dot-com.github.io/mfrmr/reference/fit_mfrm.md) -\> [`mfrm_results()`](https://ryuya-dot-com.github.io/mfrmr/reference/mfrm_results.md) | `summary(res)`, `plot(res, type = "qc")`, `summary(res)$next_actions` |
...
| One-call HTML bundle from a fit | `export_mfrm_bundle(fit, include = c("core_tables", "checklist", "apa", "summary_tables", "manifest", "script", "html"))` | Computes missing diagnostics and writes a local HTML/CSV/replay bundle without requiring a prior [`mfrm_results()`](https://ryuya-dot-com.github.io/mfrmr/reference/mfrm_results.md) object |
| Candidate-model comparison | `compare_mfrm(...)` -\> `build_model_choice_review(...)` | Convert the review with [`build_summary_table_bundle()`](https://ryuya-dot-com.github.io/mfrmr/reference/build_summary_table_bundle.md) for appendix/export handoff; treat bounded `GPCM` as sensitivity evidence unless the scoring argument justifies reweighting |
...
Before branching into specialist helpers, keep the 0.2.2 boundary
summary in view:
...
opening the detailed tables. `summary(report)` is the short
reader-facing version of that surface: it lists the immediate actions,
optional not-requested sections, claim-readiness counts, report gaps,
and wording boundaries without adding a new pass/fail rule. HTML output
uses the same order, placing reader guidance and report-summary tables
before the full Markdown text. `report_index` then shows the major
evidence areas, status, readiness label, review-signal count, and
primary/template tables to inspect next; `claim_readiness` and
`report_gaps` show which claims are ready, which need caveats, and which
require a more specific `include` preset or helper. `report_index` also
carries `EvidenceRoute`, `TemplateRoute`, `PlotRoute`, `ExportRoute`,
and `IncludePreset` columns so the report route points to the next
table, figure, export, or `mfrm_results(include = ...)` call without
turning those routes into new evidence. `template_index` stacks all
reporting-template rows across fit, precision, bias, misfit/pathway, and
linking/anchor areas so unsupported or caveated wording can be reviewed
before opening the full template text. Detailed tables remain available
through `report$tables`; use `report$report_index$PrimaryTable`,
...
Inside `summary(res)`, start with `triage` before reading every table.
It orders unavailable, review, informational, and OK signals across
diagnostics, plots, tables, precision/reliability, reporting, model
scope, and network review surfaces.
...
- bias and DFF workflows through
...
complete-case or score-support warnings. This branch does not currently
provide Wald-test or p-value reporting, multidimensional latent
regression, or posterior predictive checking; use it as a transparent
one-dimensional `MML` population-model route rather than a general
latent-regression framework.
...
- Not supported in this release: full FACETS-style score-side contract
  ...
  diagnostics are available; it remains caveated because those fields
  are not FACETS-equivalent score-side standard errors or operational
  score-scale decisions. APA/QC/export bundles and linking review are
  also available only as caveated GPCM reporting / exploratory review
  surfaces with explicit boundary output.
...
route from full FACETS-style score-side review.
...
public workflow, bounded-`GPCM` recovery-review refinements, sparse
linked designs, peer-review design review, and release-engineering
gates; the external common-data recovery summary remains the 0.2.0
artifact until that separate workflow is refreshed.
...
For manuscript or reviewer handoff, keep candidate-model comparison in
...
Use `cmp` to inspect whether log-likelihood, AIC, and BIC are on a
comparable basis. Use `review$model_roles` and `review$report_templates`
to explain which fit is the equal-weighting operational reference and
which fit is a slope-aware sensitivity model. Use `model_choice_tables`
for APA table handoff, appendix export, or inclusion in an HTML bundle.

Do not report bounded `GPCM` as the operational winner solely because
fit improved. A defensible report needs the comparison basis, the score
...
beside the plotted values. For appendix handoff, `summary(diag_eval)`,
`build_summary_table_bundle(diag_eval)`, and
`export_summary_appendix(diag_eval, preset = "recommended")` return the
same scenario, performance, report-signal, contrast, and draw-free
...
is data generation, design/recovery evaluation, diagnostic screening,
appendix export, or network/peer-review design review.
...
- `load_mfrmr_data("example_bias")`: compact example with known
  `Group x Criterion` differential-functioning and `Rater x Criterion`
  interaction signals for bias-focused help pages.
...
If your endpoint is a manuscript or internal report, use the
...
- For analytic G/D-study projections, read `IdentificationStatus` and
  the status columns before using the numeric bands. Boundary or
  singular mixed-model fits are kept as design-identification warnings,
  so a large `G` or `Phi` does not automatically become
...
be reached from `summary(res)`, `summary(report)`, or a scoped guide
rather than chosen from the namespace by name.
...
mixture or response-time model.
...
with the installed package. Open it with:
