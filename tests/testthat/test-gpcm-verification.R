...
test_that("GPCM public results, APA report, export, and replay stay connected", {
  ...
  connected_status <- results$status$Status[
    match(connected_sections, results$status$Section)
  ]
  expect_identical(connected_status, rep("ok", length(connected_sections)))
  ...
  expect_equal(
    as.numeric(replay$fit$population$parameters$Estimate),
    as.numeric(.gpcm_fit$population$parameters$Estimate),
    tolerance = 1e-6
  )
})
