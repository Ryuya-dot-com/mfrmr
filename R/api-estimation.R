...
#'   boundary. A separate workload-bounded check can identify competitive
#'   paths in which additive coordinates and a constant sum-zero log-slope
#'   rate vector move together. Its canonical candidates partition levels into
#'   positive, zero, leading-negative, and deeper-negative groups; ordered
#'   `+1/-1` pairs are exact special cases. `gpcm_joint_boundary` retains pair
#'   results in `certificates` and the additional results in
#'   `rate_certificates`. A positive warning remains `not_evaluated`, with no
#'   primary value, because it is not a global result for the non-concave GPCM
#'   likelihood. Failure to find a path is likewise not promoted to a finite
#'   joint-GPCM claim; curved and rate-nonconvergent paths remain outside the
#'   audit.
#'   `config$boundary_audit$gpcm_fixed_objective_classification` combines the
#'   two path audits under the identified, unpenalized, no-finite-box JML
#'   objective. It distinguishes positive certificates, a finite retained
#'   optimizer trace with no path certified in the audited families, numerical
#'   indeterminacy, and non-evaluation while keeping global finite-maximum and
#'   global boundary-absence flags false.
#'   `config$boundary_audit$gpcm_asymptotically_affine_transport` carries an
#'   already certified constant-rate boundary and its analytic limit to curved
#'   perturbations whose additive-coordinate residual and sum-zero expanded-
#'   log-slope residual both converge to zero. This is a positive continuity
#'   result only. Bounded residuals that do not vanish, rate-nonconvergent
#'   paths, general curved-path absence, and MML remain unclassified.
#'   `config$boundary_audit$gpcm_boundary_compactification` records the
#'   finite-dimensional subsequence reduction for arbitrary unbounded JML
#'   parameter sequences. Every nonzero limiting expanded log-slope direction
#'   has a finite positive/zero/leading-negative/deeper-negative primary role
#'   pattern. This completes only the structural role enumeration: zero
#'   primary-rate coordinates and the all-zero slope-primary sector can carry
#'   slower divergent scales, so secondary hierarchies, nonconvergent-path
#'   likelihoods, general curved paths, and global boundary absence remain
#'   unclassified.
#'   `config$boundary_audit$gpcm_rate_hierarchy` records that a declared
#'   lexicographic family of expanded sum-zero log-slope coefficients can
#'   resolve all slope levels in at most `J-1` nonzero stages for `J` levels.
#'   Each stage acts only on levels that were zero at every faster stage; its
#'   active restriction need not itself sum to zero because slower
#'   identification compensation can occur in already resolved levels. This
#'   is a structural finite-depth theorem, not extraction of a hierarchy from
...
#'   unclassified.
#'   `config$boundary_audit$gpcm_parameter_sequence_flag` applies compactness
#'   after the exact nonlinear GPCM parameter-to-logit map. Category logits are
#'   reduced to contrasts relative to category zero. Every parameter sequence
#'   then has a further subsequence whose finite-dimensional contrast image is
#'   convergent or has a finite scale-separated flag, finite base, and `o(1)`
#'   contrast remainder. An internal exact lexicographic oracle classifies the
#'   row and joint-likelihood limit along such a declared flag without assuming
#'   power-law scales. The fit declares no sequence and extracts no flag.
#'   Convergence of the original sequence, equality across subsequences,
#'   competitiveness, global boundary status, existence of a finite maximum,
#'   MML, uncertainty, readiness, and external comparison remain unclassified.
#'   `config$boundary_audit$gpcm_global_existence` applies the P2f global
#'   adjudication contract. A parameter-reachable divergent path with exact
#'   likelihood limit zero certifies the universal supremum and finite-JMLE
#'   nonexistence. A free extreme-Person ray or certified global additive
#'   recession cone also proves nonexistence by strict improvement from every
#'   finite point, without necessarily identifying the supremum. Finite
#'   existence would follow from a finite point strictly above a verified
#'   complete boundary-limsup envelope, but P2e does not enumerate that
#'   envelope. Therefore absence of a positive recession certificate and a
#'   finite optimizer trace leave finite attainment open. The audit changes no
#'   readiness, uncertainty, MML, or external-comparison decision.
#'   `config$boundary_audit$gpcm_exponential_balance` applies the P2g exact
#'   exponential slope--utility balance contract. A finite exponential sum in
#'   retained free-additive coordinates is combined with an affine sum-zero
#'   log-slope path; equal combined exponents are aggregated before positive
#'   rates are passed to the P2e contrast-flag oracle, the zero rate becomes
#'   the finite base, and negative rates vanish. A divergent parameter path
#'   with a bounded response-contrast image refutes properness of the
#'   parameter-to-contrast map. Production certifies the canonical all-zero-
#'   utility witness only when the affine additive offset is structurally and
...
#'   A repository theorem-only P2i contract completes that same minimal binary
#'   operator. For owner differences `d1 = z1 - z2` and `d2 = z3 - z4`, the
#'   finite response image is exactly the two same-sign interiors plus their
#'   common zero-difference intersection; its closure adds the two axes where
#'   exactly one difference is zero. With positive success and failure mass in
#'   all four cells, every unbounded contrast path has likelihood limit minus
#'   infinity, and pooling the constrained pair on each axis gives the complete
#'   escaping-sequence likelihood envelope. Integer expanded-row counts
#'   `(2, 4, 1, 1)` versus `(1, 1, 1, 1)` put the independent optimum
#'   `(log(2), log(4), 0, 0)` on a missing axis, proving finite-JMLE
#'   nonexistence without ordinary cell separation. An internal current-fit
#'   wrapper accepts only the exact unanchored two-Person/two-owner operator.
#'   The result is not a general GPCM closure theorem and changes no stored
#'   production readiness, primary estimate, uncertainty, MML, or comparison.
#'   A repository theorem-only P2j contract then replaces the four-cell
#'   identity by a general fixed zero-offset operator. Finite response-image
#'   membership is equivalent to positive inverse-slope feasibility
#'   `diag(z) P w = A beta`. Simplex normalization of `w` proves that every
#'   finite-image closure point has a nonnegative owner-face witness, and all
#'   nonempty faces are enumerated within declared workload caps. A separate
#'   linear first-order lift constructs exact finite paths for certified proper
#'   faces and recovers all P2i strata. A three-owner rank-one counterexample
#'   shows that nonnegative face feasibility alone is not sufficient for the
#'   general closure, so unlifted faces remain open. The contract requires zero
#'   affine offset and does not complete higher-order lifts, the general
#'   likelihood envelope, readiness, inference, MML, or comparison.
#'   A repository theorem-only P2k contract completes targetwise closure
#'   membership for the workload-admitted zero-offset operator. Semialgebraic
#'   curve selection and Puiseux leading terms reduce every boundary curve to
#'   an ordered hierarchy of inverse-slope owner rates. The lower additive
#'   coefficients vanish on each owner's rows and the leading coefficient
#'   reproduces the target; these are linear conditions, and every feasible
#'   hierarchy gives an explicit product-one finite path. Exhaustively excluding
#'   all ordered hierarchies excludes the target from the closure. P2k resolves
#'   two first-order-open rank-one faces with genuine higher-order paths and
#'   excludes the P2j `(1, -2, 0)` outer-only target. It preserves earlier P2j
#'   closure certificates under numerical indeterminacy and does not construct
#'   symbolic whole-operator strata, the boundary likelihood envelope, finite-
#'   JMLE existence, readiness, inference, MML, or comparison.
#'   A repository theorem-only P2l contract connects that targetwise closure
#'   result to the positive-mass ordered-category likelihood. For observation
#'   masses `m_ik > 0`, the unique unconstrained response optimum has adjacent
#'   contrasts `z_ih = log(m_i,h+1 / m_ih)` and value
#'   `sum_ik m_ik log(m_ik / sum_k m_ik)`. Positive mass makes the response
#'   likelihood strictly concave and sends every unbounded contrast sequence
#'   to minus infinity. If P2j/P2k places this saturated target in the finite
#'   image, P2l certifies a finite global JMLE. If it lies in the closure but
#'   not the finite image, the same value is a nonattained global supremum and
#'   an explicit product-one path proves finite-JMLE nonexistence even though
#'   every category has positive mass. If the target is outside the closure,
#'   the constrained maximum exists strictly below the saturated bound, but
#'   its location, value, and finite representability remain open. This is a
#'   grouped repeated-row representation of the existing ordered likelihood,
#'   not a nominal multinomial, count, or new frequency-response family. P2l
#'   requires a fixed zero-offset operator, is not yet applied to production
#'   fits, and changes no readiness, inference, MML, FACETS comparison, or
