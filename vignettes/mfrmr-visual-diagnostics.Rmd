...
The setup below uses `JML` for a fast visual walkthrough. For final
manuscript reporting, re-run the same route with `method = "MML"` and
`diagnose_mfrm(..., diagnostic_mode = "both")` so strict marginal follow-up
and formal precision wording remain visible.
...
visual_reporting_template()[, c(
  "FigureFamily", "PrimaryHelper", "ReadFirst", "NextLook", "ReportDecision"
)]
```
...
- First read: targeting and spread. Next look: category curves and fit
  diagnostics. Report only the shared-scale description; do not treat the
  Wright map as proof of model fit.
...
- First read: whether categories progress in order. Next look: CCC or the
  category-support table when a category is sparse or retained with zero
  frequency. Report descriptive category functioning, not rating-scale
  validity by itself.
...
- First read: repeated patterns by person, rater, criterion, or score
  category. Next look: displacement, residual matrix, and the underlying
  table. Report this as case-review screening evidence.
...
- First read: which levels would move most if freed. Next look: anchor status,
  connectivity, and fit diagnostics. Avoid claiming that displacement alone
  proves anchor failure.
...
- Report decision: include strict marginal figures only when the fitted run
  actually returned strict bundles; otherwise state that only the legacy
  residual route was inspected.
...
- First read: whether forms, waves, or subsets are connected by enough common
  elements. Next look: anchor drift and information curves before comparing
  cross-subset estimates.
...
- Report decision: residual PCA is follow-up evidence for residual structure,
  not a standalone dimensionality test.
...
- First read: which facet level concentrates the flagged cells. Next look:
  low-count cells, design support, and the `estimate_bias()` table before
  wording a substantive interaction claim.
...
- `plot_local_dependence_heatmap(fit, diagnostics)` renders a Q3-style
  residual-correlation heatmap for exploratory local-dependence review.
- `plot_reliability_snapshot(fit, diagnostics)` summarizes separation,
  strata, and reliability by facet on one small page.
- `plot_residual_matrix(fit, diagnostics)` shows person x facet-level
  standardized residual aggregates for local sign/magnitude follow-up.
- `plot_shrinkage_funnel(fit_shrunk)` compares raw and empirical-Bayes
  shrunken facet estimates after `apply_empirical_bayes_shrinkage()`.
...
4. `plot(fit, type = "wright")`, `plot(fit, type = "pathway")`, and `plot(fit, type = "ccc")` for targeting and scale interpretation.
5. `plot_residual_pca()`, `plot_bias_interaction()`, and `plot_information()` for deeper structural review.
6. `plot_guttman_scalogram()`, `plot_residual_qq()`,
   `plot_local_dependence_heatmap()`, `plot_reliability_snapshot()`,
