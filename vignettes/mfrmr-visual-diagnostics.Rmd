...
facet/step uncertainty visible. For a FACETS-facing handoff, switch only the
renderer. The fitted coordinates and SE source do not change.
...
lines. It is FACETS-style visual parity, not a claim that `mfrmr` and FACETS
...
required for a numerical-equivalence statement. Use `draw = FALSE` and inspect
the `facets_style` tables when rebuilding the display with ggplot2 or another
graphics system.
