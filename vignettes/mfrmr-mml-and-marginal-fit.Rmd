...
and the slope-aware element-conditional `fair_average_table()` and
`estimate_bias()` (with an explicit `caveat` field reminding users that the
SE columns are scaled facet-measure SEs, not delta-method SEs of the
fair-average / bias values). `build_visual_summaries()` and
`run_qc_pipeline()` route those caveated screens into review bundles.
Broader APA/report bundles, FACETS-compatibility score-side exports,
linking synthesis, and planning / forecasting helpers remain out of scope
for `GPCM` in this release. See `gpcm_capability_matrix()` for the full
...
This is also why score-side compatibility-contract outputs (FACETS parity,
score-side `facets_output_file_bundle()`, manuscript-grade APA bundles)
remain out of scope for bounded `GPCM` in this release: their published
forms are Rasch-family score transformations, and the slope-aware analogue
that would replace them in a free-discrimination context requires
delta-method standard-error machinery that is not yet exposed.
`fair_average_table()` and `estimate_bias()` themselves are now unblocked
under bounded `GPCM` via the slope-aware element-conditional kernel, with
the explicit SE caveat noted above.
