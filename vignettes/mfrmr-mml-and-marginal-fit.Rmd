...
These checks target `RSM`, `PCM`, and the bounded `GPCM` route with explicit
caveats. For `GPCM`, mfrmr supports fitting, slope summaries, posterior
scoring, information curves, direct curve/category reports, exploratory
residual-based follow-up, planning / forecasting simulations, and the
slope-aware element-conditional `fair_average_table()` and `estimate_bias()`.
`build_apa_outputs()`, `build_mfrm_manifest()`, `build_mfrm_replay_script()`,
`export_mfrm_bundle()`, `build_visual_summaries()`, and `run_qc_pipeline()`
route those caveated screens into review bundles. FACETS-compatibility
score-side exports and linking synthesis remain out of scope for `GPCM` in
this release. See `gpcm_capability_matrix()` for the full per-helper support
contract.
...
This is also why score-side compatibility-contract outputs (FACETS parity and
score-side `facets_output_file_bundle()`) remain out of scope for bounded
`GPCM` in this release: their published forms are Rasch-family score
transformations. APA/export/replay bundles are available as package-native
caveated outputs, while `fair_average_table()` and `estimate_bias()` use the
slope-aware element-conditional kernel with the explicit SE caveat noted
above.
