...
Model (GPCM; Muraki 1992). "Bounded" means a constrained package-native
many-facet GPCM branch, not a complete unrestricted GPCM implementation. The
current branch keeps the role-based design contract, requires an explicit
step-facet structure, keeps `slope_facet == step_facet`, and identifies
positive slopes with geometric mean one. Several downstream reporting helpers
remain restricted because score-side semantics under free discrimination
differ from the Rasch-family case. This vignette documents which helpers are
available, which are not, and what to use as a substitute when a helper is
restricted.
...
In FACETS-facing rater analyses, `model = "PCM", step_facet = "Rater"`
represents rater-specific category-use thresholds under a partial-credit /
`Specific` rating-scale structure. The same mfrmr mechanism can be pointed at
`"Item"` or `"Criterion"` when those facets carry the threshold variation.
External design-matrix software may call the corresponding threshold blocks
`rater:step`, `item:step`, or `criterion:step`; mfrmr keeps the public API in
`step_facet` terms so the FACETS migration path remains stable.
...
- `estimate_bias()` now provides bounded-GPCM conditional screening rows with
  ...
  tests.
- `analyze_dff()`, `analyze_dif()`, `analyze_dff_moderation()`,
  `analyze_dif_moderation()`, `dif_interaction_table()`, `dif_report()`,
  `plot_dif_heatmap()`, and `plot_dif_summary()` provide bounded-GPCM
  DFF/DIF screening and reporting surfaces with explicit `gpcm_boundary`
  rows.
...
score-side delta SEs where the required MML diagnostics support them.
The scorefile keeps `ScoreSideLogitSE` on the logit-side component scale and
uses `ScoreSideSE = ScoreSlope * Var * eta_se` for the expected-score scale.
Full FACETS-style score-side compatibility remains restricted because free
...
- Refit with `model = "PCM"` if the discrimination-free assumption is
  defensible for the data. The Rasch-family score-side contract and
  non-caveated operational reporting route become available, and
...
The fit, summary, residual diagnostics, information, recovery, fair-average,
conditional bias-screening, and APA/reporting helpers all run under `GPCM` with
the caveats listed above. `build_apa_outputs(fit_gpcm, diag_gpcm)` returns a
caveated sensitivity-reporting bundle with a `gpcm_boundary` table; it should
not be read as FACETS score-side equivalence, an automatic operational-scoring
decision, or design-forecasting evidence.
