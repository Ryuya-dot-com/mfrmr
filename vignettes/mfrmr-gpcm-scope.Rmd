...
unconditional MML, ability is integrated on a fixed standard-normal reference
scale, while the level-specific log slopes are additionally constrained to sum
to zero (geometric mean 1). Thus the fitted slopes are relative
discriminations with \(G-1\) free contrasts. A conventional fixed-ability-scale
GPCM in which every slope could share an estimated value other than 1 is not
contained in this branch. Under JML or active latent-regression MML, the same
geometric-mean constraint also participates in resolving the ability/slope
scale.
...
before computing differences. `immer` provides PCM-design JML/CML/CCML routes
and a hierarchical rater model, not a direct free-GPCM many-facet match. Its
role is consequently a unit-slope reduction or alternative-model sensitivity
analysis rather than free-slope numerical validation.
