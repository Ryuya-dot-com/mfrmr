...
Before treating a legacy workflow as covered, translate the FACETS-facing
vocabulary and then inspect the public coverage boundary:
...
| Models | Rating-scale, partial-credit, polytomous step models, multiple `Model=` statements, and additional scale codes | `RSM`, `PCM`, bounded `GPCM`; one response-model family per `fit_mfrm()` call |
...
name). In rater-mediated scoring, `step_facet = "Rater"` means each rater
receives their own category-use thresholds, matching the FACETS partial-credit
or `Specific` rating-scale idea. Use `step_facet = "Item"` when items carry
the distinct partial-credit structures, and `step_facet = "Criterion"` when
rubric criteria do. If a design-matrix package labels the same blocks
`rater:step`, `item:step`, or `criterion:step`, treat those as external
comparison vocabulary rather than new mfrmr arguments.

FACETS command files can go beyond this direct translation. Multiple
`Model=` statements in one analysis, `Dn` dichotomization, `M` missing-data
row matching, `Bn`/`B100` binomial trials, `P` Poisson counts, named
`Rating Scale=` blocks with command-level recoding, and `R` data replication
are not parsed by `mfrmr`. Use `facets_feature_coverage()` and
`system.file("references", "FACETS_manual_mapping.md", package = "mfrmr")`
to distinguish package-native equivalents from unsupported FACETS command
surfaces.
...
For handoff documentation, add an explicit anchor contract:

```r
anchor_contract <- anchor_linking_contract(fit)
anchor_contract$contract[, c("Surface", "Status", "Boundary")]
```

This contract is deliberately narrower than FACETS `Anchorfile=` output.
`make_anchor_table()` produces R-native `Facet` / `Level` / `Anchor` rows for
later `fit_mfrm(anchors = ...)` calls. It does not write a complete FACETS
specification file with final measures, rating-scale calibrations, current
settings, embedded data, and `,A` flags. `export_mfrm_bundle(include =
"anchors")` writes both the anchor CSV and the anchor-contract CSV so those
boundaries travel together.
...
For the broader FACETS visual surface, use the contract table:

```r
facets_term_crosswalk("visuals")[, c("FACETSTerm", "mfrmrRoute", "Boundary")]
...
This table maps FACETS Table 6/8 displays, Graphs-menu curves, DIF/bias Excel
plots, Graphfile output, and R/Web plot menus to the closest `mfrmr` visual,
plot-data, or `as_ggplot()` route. It is deliberately not a pixel-identical
FACETS graph-window, clickable Webpage, Excel workbook, or line-printer
reproduction claim.
