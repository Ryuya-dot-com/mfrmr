...
| Numerical integration and population inference | [Use-condition audit](inst/validation/mml-use-condition-audit-record-0.2.4.md), population identification/profile/output reviews and [full-information checks](inst/validation/population-full-information-record-0.2.4.md); [40 population preflight datasets](inst/validation/population-coverage-record-0.2.4.md). | Default-grid and long-pattern sensitivity remain design-specific; estimated-population inference stays restricted. The 80,000-dataset population confirmation has not run. Local full rank does not establish strong information or valid boundary inference. |
| Fair Scores and their intervals | Earlier pilot/output repairs and [frozen FairZ confirmation with 40-dataset preflight](inst/validation/fairz-coverage-record-0.2.4.md); joint-SE numerics, full refits, native source identity and resume checked. | Execute the unrun 20,000-dataset FairZ study and assess its fixed criteria. Reestimated-reference FairM and Person intervals need distinct targets; no preflight supplies coverage validation. |
...
is now frozen with 40 matching-source preflight datasets; the planned
