...
multiple observed scales come before multidimensional latent traits.
...
0.2.3 is the external-comparison and release-gate release. It should strengthen
claims about the existing model scope before introducing a new family.
It may establish fixtures and design decisions needed by later work, but it
must not advertise a new multi-scale, threshold-anchor, or calibration-bundle
API as part of this release.
...
- [ ] Classify differences as parameterization, identification, numerical,
  reporting, or genuinely unsupported rather than treating one correlation as
