...
- current model boundary: one response-model family and one observed
  score scale are used per fit; mixed families, multiple independent
  scales, general threshold anchoring, and fixed-calibration scoring are
  not part of the current public estimator;
