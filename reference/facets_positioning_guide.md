...
- current model boundary: one ordered-categorical response-model family
  ...
  general threshold anchoring, and importing FACETS or third-party
  frozen calibrations are not part of the current public estimator;
  native portable calibration is available for the narrower documented
  RSM/PCM MML route, and posterior scoring from an existing fitted
  object remains separate;
