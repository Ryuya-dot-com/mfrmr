...
The CRAN release is the baseline for examples and external review. Lightweight
examples use the bundled `example_core` data so the public route stays quick to
run, while heavier validation, simulation, and CRAN-check evidence can be
generated separately. The core reproducible route is the same public route
recommended above:
...
Longer-running checks are intentionally kept out of first-contact examples.
They are regenerated by explicit scripts and summarized through saved `.rds`
and `.csv` artifacts. The workflow vignette uses small generated CSV artifacts
to show representative output during CRAN-style builds while keeping the live
calculation route available with `NOT_CRAN=true`.
